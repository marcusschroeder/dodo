...
#hallem = Hallem()
#data = np.transpose(hallem.response) # Glomeruli x Odorants instead of
# Odorants x Glomeruli
features = np.arange(2, 40)
times = np.zeros((len(features), 4))

data = sample_generator.generate_random_data(32, 64)
...
try:
    ...
    for ii, vv in enumerate(features):
        ...
        write_csv("results/g_features_random.csv", times)
except GurobiError:
